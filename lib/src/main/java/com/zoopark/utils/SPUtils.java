...
public class SPUtils {
    ...
    private static SimpleArrayMap<String, SPUtils> sSpInstanceNameMap = new SimpleArrayMap<>();
    ...
    public static SPUtils getInstance(Context context, String spName, final int mode) {
        if (isSpace(spName)) spName = "gb_sp";
        SPUtils spUtils = sSpInstanceNameMap.get(spName);
        if (spUtils == null) {
            spUtils = new SPUtils(context, spName, mode);
            sSpInstanceNameMap.put(spName, spUtils);
        }
        return spUtils;
    }

    private SPUtils(Context context, final String spName) {
        this(context, spName, Context.MODE_PRIVATE);
    }
